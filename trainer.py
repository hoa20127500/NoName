...
from matching_flow import MatchingFlow
...
class Trainer:
    def __init__(self, dataset, params, model_name):
        ...
        # MatchingFlow loss uses pos/neg group structure, so it cannot be
        # split across GPUs by DataParallel.
        if model_name == "MatchingFlow":
    ...
    def train(self, early_stop=False):
        ...
        for epoch in range(1, self.params.ne + 1):
            ...
            while not last_batch:
                ...
                out = self.model(heads, rels, tails, years, months, days)

                if self.model_name == "MatchingFlow":
                    loss = out
                else:
                    ###Added for softmax####
                    num_examples = int(heads.shape[0] / (1 + self.params.neg_ratio))
                    scores_reshaped = out.view(num_examples, self.params.neg_ratio+1)
